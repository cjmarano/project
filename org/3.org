...
* May
* Start times around 3 seconds. New is 7+ seconds.
...
* Lisp programming.
* NEXT emacs server / client.
** emacs --daemon   emacs --fg-daemon emacsclient -cache
** client starts is 8+ Seconds. <-- using erc alias client starts instantly.
using  emacsclient --no-wait -c gives sub 3 second open of file.
...
** alias erc='emacsclient -cn <filename> &'  <-- seems to work. For intial frame after emacs --daemon.
** emacsclient --no-wait -c <filename> <-- new GUI frame.
