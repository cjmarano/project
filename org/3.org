...
* For now use New for programming don't update Big yet.
* lisp programming.
...
* Need to check contents of Elpa against package-selected-packages in init.el on both Big & New.
* removed some items from ~/.emacs.d/elpa/ on big and new. Added some to selected, ditto.
