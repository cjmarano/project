...
* Ghostty on New still won't show working directory.  Okay, now it does on New and Big.
** _in addition on Big shows icons alongside cwd.
...
Ghostty doing two line prompt but is throwing error in shell integration file.
...
* Emacs on Big takes 7 secs. to open vs. 3 for New.
** Do a diff on their init.el files.
** Check number of packages loaded.
