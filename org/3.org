...
CLOSED: [2025-10-28 Tue 09:39]
** COMPLETED Copied theme.toml from Big to New. Prefer colors
CLOSED: [2025-11-03 Mon 10:37]
* COMPLETED Freeze init.el and work on other things. freezeNew31OCT.init.el is latest good b/u.
...
* COMPLETED Removed projectile from init.el on New and Big and init.el.org
CLOSED: [2025-11-04 Tue 11:59]
** Copied both init.el files to project.
**  Weird - init.el on New lost a bunch of lines including Selected-packages. Copied b/u
init.el from ~/project/ to restore.
`* COMPLETED Can't click items in Dashboard on New with normal init.el
CLOSED: [2025-11-04 Tue 10:17]
...
** Replacing init.el on New w/ an init.el from Big solved the can't click in dashboard issue.
...so obviously something corrupted in New's init.el. That's why backup copies in Mystuff/Shell/Old inits important.
** homogulating init.el on Big w/ changes on init.el from New broke Dashboard again.
had to revert init.el on Big to earlier version. So... something foul w/ init.el on New.
** dashboard on New now working with earlier init.el
* NEXT 30OCT - 04NOV. Homogulated init.el on Big and new Broke/fixed Dashboard on New.
...
* Need to check init.el.org to see which init.el it matches and if it needs to be modified.
*  lisp programming.
...
* Upgraded pip then installed python-lsp-server:
Successfully installed black-25.9.0 click-8.3.0 docstring-to-markdown-0.17 importlib-metadata-8.7.0 jedi-0.19.2 mypy-extensions-1.1.0 packaging-25.0 parso-0.8.5 pathspec-0.12.1 platformdirs-4.5.0 pluggy-1.6.0 python-lsp-jsonrpc-1.1.2 python-lsp-server-1.13.1 pytokens-0.3.0 typing_extensions-4.15.0 ujson-5.11.0 zipp-3.23.0

Pyflakes was installed.

Installed python-lsp-ruff
Installing collected packages: ruff, attrs, cattrs, lsprotocol, python-lsp-ruff
Successfully installed attrs-25.4.0 cattrs-25.3.0 lsprotocol-2025.0.0 python-lsp-ruff-2.3.0 ruff-0.14.3

** A python file opened iwht syntax highlighting and auto completion but closong that file and opening another
python file lost syntax and completion. Problem with connecting to lsp-server?
* Rust stuff
