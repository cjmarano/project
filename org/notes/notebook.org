...
* Eval expression
C-x, e  <-- bound to  kmacro-end-and-call-macro
C-M, x <==== +WORKS+.
Alt, n <-- undefined
C-u, n  <-- Nope!
M, n <-- undefined
...
in terminal - git clean -f to remove 'ghost' files.
also see git update-index
* Grep
grep default
...
* Movement
M-g, c is M-x goto-char
M-g, g is M-x goto-line (as is S-l)
...
* Org Mode
C-x, h to select whole org buffer then M-x org-sort-entries <RETURN> then a for alphabetical.
