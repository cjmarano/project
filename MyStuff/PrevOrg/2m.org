* July
* Start times around 3.2 seconds, up from 3. Around 8 seconds for Big.
...
** changed conky argb opacity 50 to 0 now it IS on a transparent background.
=======
fooworld.lisp works. _hellowrld.lisp doesn't work_.    _main.lsip doesn't work_  sequence.lisp runs but throws error: undefined variable X.  Strings.lisp runs as a script. NOTE: same error in array.lisp whether run in clisp or SBCL.
>>>>>>> 7a03a5e (daily update of 3.org)
* ISSUES
...
CLOSED: [2025-07-07 Mon 09:23]** Kitty will scroll with C-<shift> and page up, page down, home and end. 
...
** Had to re-install elpy. Yasnippet still loading, even though all references gone from init.el
...
** circle.lisp working after changes. hello.lisp working. array.lisp working but there are errors.
...
** asdf is included whan Quicklisp is loaded. Check this on Big and New.
** Practicals asdf examples are overly complicated and annoying.
